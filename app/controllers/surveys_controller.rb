class SurveysController < ApplicationController
  ...
  def create
    ...
    if @survey.user_has_contributed_today?:
      render json: { 
          errors: "The user already contributed with this survey today" 
        }, 
    ...
    if @survey.save
      ...
      if @survey.symptom.length > 0
        render json: { 
            survey: @survey, 
            feedback_message: @user.get_feedback_message(@survey), 
            messages: @survey.get_message(@user) 
          }, 
          status: :created, 
          location: user_survey_path(:id => @user)
      end
        render json: { 
          survey: @survey, 
          feedback_message: @user.get_feedback_message(@survey) 
